...
public class ModelHelper {
    ...
    public static Player fetchPlayer(long id, ScoreDAO dao) {
        ...
        try {
            ...
            return new Player(id, name, difficulty, hiScore, lastScore, totalScore, totalPlayed);
        } finally {
            cursor.close();
        }
    }
    ...
    public static void savePlayer(Player player, ScoreDAO dao) {
        dao.savePlayer(player.getId(), player.getName(), player.getDifficulty(), player.getHiScore(),
                player.getLastScore(), player.getTotalScore(), player.getTotalPlayed());
    }
}